...
message = "Please enter the stock symbol you'd like to analyze. Enter 'DONE' when you have finsihed inputting stocks. "
...
while i < (len(sym_list)):
    ...
    request_url = f"https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol={sym_list[i]}&apikey={api_key}"
    response = requests.get(request_url)
    
    if 'Error' in response.text:
        print ("Invalid input: " + str(sym_list[i]) + ".  Exiting program...")
        exit()
    else:
        ...
        csv_file_path = os.path.join(os.path.dirname(__file__), "..", "data", f"{sym_list[i]}.csv") # a relative filepath
    ...
    #LOOP to write each row
        ...
        print("-----------------------------------")
        print(f"STOCK SYMBOL: {sym_list[i]}")
...
print("All stock data has printed")